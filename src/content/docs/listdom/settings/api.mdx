...
<CardGrid>
  <Card title="Mobile App Integration">
    A mobile app uses the API to let users register, log in, browse, and submit listings. It calls `/login` to get a user token, `/search` to display listings, and `/listings` (POST) to create new entries, providing a seamless, real-time experience synced with the WordPress backend.
  </Card>
  <Card title="Headless Website">
    A developer builds a React front-end using WordPress as a headless CMS. The app calls Listdom's API to fetch all data: `/search` for listing loops, `/taxonomies` for filter menus, and `/listings/{id}` for detail pages, while handling user authentication via the API.
  </Card>
  <Card title="Third-Party Integration">
    A script automates content updates by fetching data from an external system and using the Listdom API to add or update listings. It uses a service account to log in, gets a user token, and then calls `POST` and `PUT` on the `/listings` endpoint to sync content daily.
 </Card>
</CardGrid>
...
<LinkCard
  ...
  description="Learn more about the WordPress REST API, authentication methods, and best practices for making API calls."
/>