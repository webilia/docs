...
If you have defined custom listing fields (called Attributes in Listdom), those will appear in the Listing Information section when adding a listing. Attributes are extra fields (such as text, number, select, checkbox, etc.) that capture additional information not covered by the default fields. For example, if you run a real estate directory, you might add attributes for Bedrooms, Bathrooms, or Square Footage. Each custom attribute will be shown with its field input here.

- **Custom Attributes Fields**: These will display as input fields labeled by the attribute name. Fill in each attribute as needed. Some attributes might allow multiple selections or file uploads depending on their type. For instance, an attribute defined as an image field will let you upload an image for that listing, and a multi-select attribute will let you choose multiple options.
  <Aside type="note">
  If no custom attributes are set up, the Listing Information section will simply show a message like "No attribute is available." Custom attributes can be created via Listdom's settings or with the Listdom Custom Fields add-on.
  </Aside>
- **Required Attributes**: Administrators can configure certain fields or attributes as required. Required fields are indicated with a red asterisk (*). You must provide values for any required attribute before you can publish the listing.
