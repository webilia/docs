...
sidebar_label: Advanced Icon Addon
...
- **Map Markers:** When using map views, Listdom typically uses category icons for map markers. Custom icons will be used here as well. If “Restaurant” category has a custom icon (say a fork and knife), map pins for restaurant listings can display that icon within the marker, making the map more informative:contentReference[oaicite:12]{index=12}:contentReference[oaicite:13]{index=13}.
...
By default, if a term has no custom icon, Listdom will continue to show the standard icon or text for that term. The addon only overrides the icon where one is provided:contentReference[oaicite:14]{index=14}.
...
<CardGrid>
  ...
  <Card title="Custom Field Icons in Search">
    Using Listdom Pro’s custom Attributes, a travel directory has an attribute for **Difficulty Level** of hiking trails. The admin sets a custom icon (e.g., a mountain graphic) for this attribute. On the search filter for Difficulty, the dropdown now shows the mountain icon next to each level, thanks to the Advanced Icon Addon integrating the custom icon into the interface.
  </Card>
</CardGrid>

<LinkCard href="https://listdom.net/products/listdom-advanced-icon-addon" title="Advanced Icon Addon Official Page">
  Visit the official Advanced Icon Addon page for more details, including suggested icon formats and a visual guide on how icons are used in the Listdom theme.
</LinkCard>
