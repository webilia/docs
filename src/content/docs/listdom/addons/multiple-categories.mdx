...
**Display Multiple Categories:** *(Switch, default OFF)* - Enable this option to display all the categories assigned to a listing on the front-end, alongside the primary category. When this is turned **On**, each listing's additional categories will be visible on listing cards and single listings, typically as extra category labels or tags. If this is **Off**, the addon still allows multiple categories to be selected in the listing editor, but only the primary category will be shown to users.
