...
With the ACF Addon, Listdom’s import/export functionality will include ACF custom field data as well. When you export listings (via Listdom’s CSV export, for example), any additional ACF fields are added as extra columns in the export file:contentReference[oaicite:0]{index=0}:contentReference[oaicite:1]{index=1}. Similarly, during import, the addon attempts to map and import values into ACF fields.

- **Export Columns:** Each custom field (except layout/style fields like ACF tabs or messages) will produce a column in the CSV export named after the field’s label:contentReference[oaicite:2]{index=2}:contentReference[oaicite:3]{index=3}. This allows you to retain those custom values outside WordPress.
...
<Aside type="caution">Ensure that the destination site has the same ACF field groups configured before importing listings with ACF data. The ACF Addon uses the field names/keys to match values on import:contentReference[oaicite:4]{index=4}:contentReference[oaicite:5]{index=5}. If a field doesn’t exist on the new site, its data will be skipped.</Aside>
...
<CardGrid>
  ...
  <Card title="ACF Data Export">
    A site administrator exports listings to CSV for analysis. Because of the ACF Addon, the CSV includes columns for custom fields like **“Price Range”** or **“Organizer Name”** that were added via ACF, preserving all the listing information for external review.
  </Card>
</CardGrid>

<LinkCard href="https://listdom.net/products/listdom-acf-addon" title="Listdom ACF Addon Documentation">
  Learn more about the Listdom ACF Integration on the official website, including setup guides and best practices.
</LinkCard>
