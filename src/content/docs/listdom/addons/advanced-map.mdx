...
sidebar_label: Advanced Map Addon
...
This addon introduces a **Marker Type** setting to control what content appears as the marker on maps. By default, Listdom uses category-based markers (often an icon or pin). With Advanced Map, you can choose from several marker styles:contentReference[oaicite:15]{index=15}:
...
- **Label:** Uses the listing’s Label taxonomy (if any) as the marker. The marker will show the text of the label or a small badge colored with the label’s color:contentReference[oaicite:16]{index=16}:contentReference[oaicite:17]{index=17}. For example, if you use “Featured” labels, the marker could show a colored badge with the label name.
- **Price:** Shows the listing’s price (typically the formatted price value) inside the marker:contentReference[oaicite:18]{index=18}. This is great for directories like real estate or rentals – each property’s map pin can display its price, giving immediate context.
- **Square (Category Icon):** A unique style that shows a square marker containing the category’s icon on a colored background:contentReference[oaicite:19]{index=19}:contentReference[oaicite:20]{index=20}. This stands out on the map and is useful for emphasizing category via iconography.
- **Listing ID:** Displays the listing’s ID number in the marker:contentReference[oaicite:21]{index=21}:contentReference[oaicite:22]{index=22}. This can be useful for internal reference or when you want a neutral identifier on the map pins.
...
<Aside>
type="tip">Pairing Marker Types with custom icons/colors can improve user experience. For example, if using **Price** markers, ensure your price format is short (like “$250K”) so it fits nicely in the pin.
</Aside>

## Infowindow Style

The **infowindow** is the popup box that appears when a user clicks on a map marker, showing details about the listing. The Advanced Map Addon provides an **Infowindow Style** setting to change its design:contentReference[oaicite:23]{index=23}:

- **Style 1:** The default Listdom infowindow style (a simple layout, usually listing title and maybe thumbnail).
- **Style 2:** An alternative layout provided by the addon. Style 2 offers a different design, perhaps larger or with additional info. Enabling Style 2 will use a template specifically crafted for a richer info popup:contentReference[oaicite:24]{index=24}:contentReference[oaicite:25]{index=25}. For example, it might include extra details like price or address in the popup.
- **Custom (Builder Template):** The addon can integrate with page builders (Elementor/Divi) to allow a fully custom infowindow design. In the Infowindow Style dropdown, any saved custom infowindow layouts will appear (usually as numeric IDs or names). Selecting one will use that design for your map’s infowindows:contentReference[oaicite:26]{index=26}. This means you can design an infowindow as a small card using your page builder and have it show for each listing.
...
<Aside>
type="note">If you choose a **Custom Builder** infowindow (for example, an Elementor template), make sure that template is designed to display dynamic listing content. The addon adds “Infowindow” as a layout context in supported builders, so you can create a template with Listing dynamic fields and assign it here.
</Aside>
...
Advanced Map Addon adds a feature to **exclude certain listings from maps** based on category. In the settings (Advanced Map → *Map Exclude*), you can check off specific categories under “Select the categories to remove from the map.”:contentReference[oaicite:29]{index=29}. Any listing belonging to an excluded category will not be shown on any map views.
...
<Aside>
type="tip">Use the exclude feature sparingly – it’s generally better to include as many relevant listings on the map as possible for user completeness. Only exclude categories that truly don’t make sense on a map or are meant to be hidden.
</Aside>

## Restricting Map Bounds

The addon provides **Map Bounds** controls to limit the area users can pan or zoom to on your maps. In settings, under *Bounds → Restrict Bounds*, you can enter a **South West** and **North East** latitude/longitude to define the bounding box:contentReference[oaicite:30]{index=30}:contentReference[oaicite:31]{index=31}.
...
- **How to Set:** Input the lat/long of the South-West corner (bottom-left) and North-East corner (top-right) of the region you want to confine the map to. A tool like **latlong.net** can help find coordinates:contentReference[oaicite:32]{index=32}. Once set, users will be unable to pan outside this box; the map will “bounce” them back if they try.
...
For Google Maps users, the addon enables adding unlimited **Map Styles** (e.g., Snazzy Maps JSON styles). In *Advanced Map settings*, there’s a **Map Styles** section where you can create styled map presets:contentReference[oaicite:33]{index=33}:contentReference[oaicite:34]{index=34}.

- **Adding a Style:** Click **“Add Style”** in the Map Styles section. This will create a new entry where you can provide a Title (for your reference, e.g., “Night Mode Map”) and a JSON style snippet:contentReference[oaicite:35]{index=35}:contentReference[oaicite:36]{index=36}. The JSON should be the styling rules for Google Maps (commonly sourced from Google’s style tool or SnazzyMaps).
...
- **Internal Direction Module:** In settings, a toggle **“Internal Module”** can be turned on:contentReference[oaicite:37]{index=37}. When enabled, the listing detail page will show an interactive directions interface (for example, a text box for the user to enter their starting location or a “Get Directions” button). This uses Google’s Direction API behind the scenes. Users can generate a route from their location (or entered address) to the listing’s address without leaving your site. <Aside> type="caution" title="Google API Key">Using the internal directions requires that you have enabled the **Google Directions API** on your Google Cloud project and provided a valid API key in Listdom’s settings:contentReference[oaicite:38]{index=38}. Without this, the feature may not function.</Aside>
- **Infowindow Direction Link:** Another setting, **“Infowindow Link”**, if switched on, adds a direct link or button inside the infowindow (Style1 and Style2) that says something like “Directions”:contentReference[oaicite:39]{index=39}. Clicking this could open Google Maps in a new tab with the destination pre-filled to the listing’s location (so the user can quickly get directions via Google Maps app/website).
- **Listing Address Link:** The **“Link Address”** option will turn the listing’s displayed address on the listing page into a clickable link to Google Maps directions:contentReference[oaicite:40]{index=40}. Essentially, if a user clicks the address, it will open an external Google Maps page showing the route from their current location (if available) to that address. This is a simpler alternative to the full internal module if you prefer not to embed route functionality.
...
<Aside>
type="tip">The **infowindow link** and **address link** do not require heavy setup beyond having a Google Map on the page, since they essentially redirect to Google Maps. These are quick wins for user experience if you don’t want to set up the full directions API. The internal module is more interactive but needs the API key.
</Aside>
...
<LinkCard href="https://listdom.net/products/listdom-advanced-map-addon" title="Advanced Map Addon Documentation">
  See the official Advanced Map Addon documentation for code examples of custom map styles and further guidance on Google Maps API setup.
</LinkCard>
