...
<Aside type="tip"> Pairing Marker Types with custom icons/colors can improve user experience. For example, if using **Price** markers, ensure your price format is short (like “$250K”) so it fits nicely in the pin.
</Aside>
...
<Aside type="note"> If you choose a **Custom Builder** infowindow (for example, an Elementor template), make sure that template is designed to display dynamic listing content. The addon adds “Infowindow” as a layout context in supported builders, so you can create a template with Listing dynamic fields and assign it here.
</Aside>
...
<Aside type="tip"> Use the exclude feature sparingly – it’s generally better to include as many relevant listings on the map as possible for user completeness. Only exclude categories that truly don’t make sense on a map or are meant to be hidden.
</Aside>
...
<Aside type="tip"> The **infowindow link** and **address link** do not require heavy setup beyond having a Google Map on the page, since they essentially redirect to Google Maps. These are quick wins for user experience if you don’t want to set up the full directions API. The internal module is more interactive but needs the API key.
</Aside>
