...
The **Listdom Divi Addon** allows you to harness the power of the Divi Builder to create custom layouts for your directory’s listings. With this addon, you can design the **Single Listing** page visually using Divi’s drag-and-drop interface, and even create custom listing card designs. The addon provides a suite of Divi modules (visual building blocks) for every Listdom listing element – title, description, gallery, map, features, and more – so you can lay out and style listing content exactly as you wish:contentReference[oaicite:78]{index=78}:contentReference[oaicite:79]{index=79}.
...
Once the Divi Addon is activated, no additional settings are needed in Listdom (the Addon’s panel will simply inform you to use Divi’s tools):contentReference[oaicite:80]{index=80}:contentReference[oaicite:81]{index=81}. You will primarily work within **Divi → Theme Builder** to create templates for Listdom listings:
...
- **Listing Title:** Displays the listing’s title. You can choose the heading level (H1, H2, etc.) and style it:contentReference[oaicite:82]{index=82}:contentReference[oaicite:83]{index=83}.
...
- **Listing Map:** Displays a map with the listing’s location pin:contentReference[oaicite:84]{index=84}:contentReference[oaicite:85]{index=85} (will only show if the listing has an address/coordinates).
- **Listing Address:** The text address of the listing:contentReference[oaicite:86]{index=86}.
- **Listing Features:** A list of the listing’s features or amenities:contentReference[oaicite:87]{index=87}.
- **Listing Price:** The price or price range of the listing, formatted with currency:contentReference[oaicite:88]{index=88}.
- **Listing Price Class** <Badge text="Pro" />: Displays a price category or label (if using a pricing taxonomy):contentReference[oaicite:89]{index=89}.
- **Listing Owner:** Information about the listing author/owner (name, avatar, etc.):contentReference[oaicite:90]{index=90}.
...
- **Listing Breadcrumb:** A breadcrumb navigation showing the listing’s position (e.g., Home > Category > Listing):contentReference[oaicite:91]{index=91}.
...
- **Listing Booking** – Requires Listdom Booking Addon (shows booking form/details):contentReference[oaicite:92]{index=92}.
- **Listing Auction** – Requires Listdom Auction Addon (displays auction bid info).
- **Listing Claim** – Requires the Claim/Verification Addon (for “claim this listing” status):contentReference[oaicite:93]{index=93}.
- **Listing ACF** – Requires the ACF integration Addon (shows custom fields):contentReference[oaicite:94]{index=94}.
- **Listing Favorite** – Requires the Favorites feature (e.g., part of <Badge text="Pro" /> plan, to show a favorite button).
- **Listing Stats** – Requires Listdom Pro’s statistics (views count) to be enabled:contentReference[oaicite:95]{index=95}.
- **Listing Franchise** – Requires Franchise/Branch Addon if applicable:contentReference[oaicite:96]{index=96}.
  
These modules will only appear in Divi if you have the corresponding addon active:contentReference[oaicite:97]{index=97}. The Divi Addon intelligently registers them as needed.
</Aside>
...
Each module comes with the usual Divi design settings. You can set fonts, colors, backgrounds, etc., and the module will render the actual listing data on the front-end. If a certain listing field is empty (e.g., no price given), the module will typically not display anything (or show a placeholder like “No listing available” which you can style):contentReference[oaicite:98]{index=98}:contentReference[oaicite:99]{index=99}.

## Custom Listing Cards (Templates)

In addition to single pages, the Divi Addon can help design listing **cards** (the listing preview boxes in grid or list views). The addon registers a custom post type **Divi Templates** in wp-admin (found under Listdom as **Divi Templates**):contentReference[oaicite:100]{index=100}. You can create a new Template here and design it with Divi Builder to serve as a listing card layout. Key steps:

- Go to **Listdom → Divi Templates** and click **Add New Template**. Give it a name like “Custom Listing Card”.
- In the Template edit screen, you can choose a **Template Type** (if available) such as “Listing Card” (for archive card views) or “Single Listing” (for full page):contentReference[oaicite:101]{index=101}. Select **Listing Card**.
