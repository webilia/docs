...
The **Listdom SMS Addon** integrates text messaging into your directory. It allows the system to send SMS notifications (for example, when certain events happen like a new listing or booking) and enables **mobile number verification** for users via SMS. The addon uses **Twilio** as the SMS gateway to send out messages:contentReference[oaicite:140]{index=140}:contentReference[oaicite:141]{index=141}. With this addon, you can improve communication by reaching users on their phones and add an extra layer of trust through phone verification.
...
- **SMS Gateway:** *(Dropdown, default Twilio)* – Currently, Twilio is the supported gateway:contentReference[oaicite:142]{index=142}:contentReference[oaicite:143]{index=143}. This field is pre-set to **Twilio**.
- **Account SID:** *(Text)* – Your Twilio Account SID:contentReference[oaicite:144]{index=144}:contentReference[oaicite:145]{index=145}. You obtain this from your Twilio console dashboard.
- **Auth Token:** *(Text)* – Your Twilio Auth Token:contentReference[oaicite:146]{index=146}:contentReference[oaicite:147]{index=147}. Together with SID, this credential allows Listdom to connect to your Twilio account.
- **Sender:** *(Text)* – The Twilio phone number (or sender ID) from which SMS will be sent:contentReference[oaicite:148]{index=148}:contentReference[oaicite:149]{index=149}. It must be in E.164 format (e.g., +16175551212). This is the number users will see as the sender of the SMS.
...
- **Message:** *(Textarea, default "Your verification code is: #code#")* – The template of the SMS sent to users during mobile verification:contentReference[oaicite:150]{index=150}:contentReference[oaicite:151]{index=151}. Use **#code#** as a placeholder; the addon will replace it with the actual numeric verification code. You can customize the wording, but ensure to keep the `#code#` token in the text. For example: "Welcome to [Your Site], your code is #code#".
...
- The content of the SMS is taken from the **SMS Content** field of that notification, which can contain placeholders like #listing_title#, #date#, etc. The addon processes these placeholders similarly to email content:contentReference[oaicite:152]{index=152}:contentReference[oaicite:153]{index=153}.
- When the event triggers, Listdom calls the SMS addon to send the message. The addon checks that the notification is marked active and SMS enabled:contentReference[oaicite:154]{index=154}:contentReference[oaicite:155]{index=155}, then formats the content and sends via Twilio:contentReference[oaicite:156]{index=156}:contentReference[oaicite:157]{index=157}.
...
<Aside type="note">
All SMS sending is done asynchronously via WordPress AJAX/cron events to avoid slowing the site:contentReference[oaicite:158]{index=158}:contentReference[oaicite:159]{index=159}. If an SMS fails (e.g., invalid number), it will be logged in the Listdom log (and Twilio’s logs).
</Aside>
...
- **Verification Shortcode:** The SMS addon provides a verification form that you can display by adding the attribute `sms-verification="1"` to any Listdom shortcode that requires a logged-in user action:contentReference[oaicite:160]{index=160}:contentReference[oaicite:161]{index=161}. For example, you could require phone verification before showing the Add Listing form: `[listdom-dashboard sms-verification="1"]`. In this case, if the user’s phone is not verified, they will see a verification form instead of the listing form.
- **Verification Form:** The form (displayed when required) will prompt the logged-in user to enter a mobile number and send a code. It has two steps:
  1. **Send Code:** User enters their phone number and clicks “Send Code”. The addon generates a 6-digit verification code and sends an SMS with the template you set (replacing #code# with the code):contentReference[oaicite:162]{index=162}:contentReference[oaicite:163]{index=163}. The code is stored hashed (md5) in the user meta and the number is temporarily stored as well:contentReference[oaicite:164]{index=164}:contentReference[oaicite:165]{index=165}.
  2. **Verify Code:** User then enters the code they received and submits. The addon checks the code against the stored hash:contentReference[oaicite:166]{index=166}:contentReference[oaicite:167]{index=167}. If it matches, verification is successful.
- Upon successful verification, the user’s mobile number is saved permanently in their profile (`lsd_mobile`) and a flag `lsd_mobile_verified` is set:contentReference[oaicite:168]{index=168}:contentReference[oaicite:169]{index=169}. From then on, that user is considered phone-verified.
...
- The next time the user visits a shortcode with `sms-verification="1"`, since they are already verified, the content will show normally:contentReference[oaicite:170]{index=170}.
...
2. **Configure SMS Settings:** In **Listdom > Settings > Addons > SMS**, input the Twilio SID, Token, and Sender number:contentReference[oaicite:171]{index=171}:contentReference[oaicite:172]{index=172}. Double-check for typos. Write a friendly verification message if you want to customize it (ensure `#code#` is included).
3. **Enable Notifications for SMS:** Decide which events warrant an SMS. Go to **Listdom > Notifications** and edit a notification (for example, "New Listing Submitted" or "Booking Confirmation"). There will be an **SMS Content** box and an **SMS Status** toggle. 
   ...
   - Craft a concise SMS Content message. Keep it under 160 characters if possible to avoid multipart SMS. You can use placeholders like #listing_title#, #name#, etc., which will be replaced:contentReference[oaicite:173]{index=173}:contentReference[oaicite:174]{index=174}.
