---
title: Functionality & Integration Issues - Listdomer Theme
description: Address problems with theme features, Listdom add-ons, and other integrations.
sidebar:
  label: Functionality & Integration Issues
  order: 4
header_button:
  label: View Demo
...
- **"Add Listing" Button does nothing:** If a user is not logged in, clicking "Add Listing" might redirect to the login/register depending on your Listdom settings. Check that the target page is set. In Header Settings, see Login Redirection Page and Register Redirection Page - set those appropriately. Ensure the Listdom plugin's settings allow front-end submissions.
- **Login/Register links not showing:** They will only show if you enabled them in Header Settings and Listdomer detects the relevant plugin. If you're not using a login plugin, they still link to the default login page. If they're not showing, double-check the setting and view the site as a guest.
...
- **Breadcrumb not showing:** Make sure Breadcrumb is enabled in Page Heading settings for the context you're testing. The breadcrumb will not show on the front page by default. On listing single pages, ensure Listdomer's breadcrumb integration is active.
...
- **Need to Reset Settings:** If you heavily modified settings and want to revert to demo defaults, re-import the Redux settings from the demo or use the Redux panel's reset options. Always back up your settings before resetting.
...
<CardGrid>
  <Card>**Add Listing Button Redirect** - Set Login Redirection Page and Register Redirection Page in Header Settings so clicking "Add Listing" sends users to the correct dashboard.</Card>
