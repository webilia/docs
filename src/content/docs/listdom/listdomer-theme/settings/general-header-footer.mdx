---
title: General, Header & Footer Settings - Listdomer Theme
...
<Aside type="note">Many options are pre-set if you imported a demo. You can modify them here. All changes are saved in the database (no code editing needed). Pro-only options are marked with a badge in the interface.</Aside>
...
- **Per-Page Header Overrides:** On any Page edit screen, you'll find a Listdomer meta box that lets you set a specific Header Type for that page. Select the desired header or use "Inherit" to use the global default. You can disable the header on a particular page by choosing the "Disabled" option.
...
<CardGrid>
  <Card>**Changing the Header Style** - In *Header Settings*, select **Header Type 3** and save. Your site header instantly changes to the new layout. If Header Type 3 is a feature, make sure Listdomer Pro is active.</Card>
