...
The **Page Title** widget dynamically displays the title of the current page or post. This widget is primarily used in templates for static pages (like About Us, Contact, etc.) or as a general solution to show the title of whatever content is being viewed. Unlike the [Archive Title](/vertex/widgets/dynamic-posts-templates/archive-title/) (for archive pages) or [Post Title](/vertex/widgets/dynamic-posts-templates/post-title/) (for singular posts), the Page Title widget is a more general-purpose title element that can work in many contexts, including pages, posts, and even archives, adjusting its output accordingly.

<Aside type="note" title='When to use'>
Use Page Title in page templates or when you need a catch-all title that works for various content types. For single blog posts, you would typically use the dedicated [Post Title](/vertex/widgets/dynamic-posts-templates/post-title/) widget (which outputs the post title without any prefix). For archive pages, use [Archive Title](/vertex/widgets/dynamic-posts-templates/archive-title/). Page Title is versatile but make sure you're using the widget best suited for your template for optimal control.
</Aside>
...
- **Dynamic Title Rendering:** The widget will display the exact title of the current page or post. If placed on a normal WordPress page, it shows that page's title. If placed on a blog post, it will show the post title. If placed on an archive or special page, it will fall back to showing the archive title (though using the [Archive Title](/vertex/widgets/dynamic-posts-templates/archive-title/) widget is recommended for those cases).
- **Include Context:** Page Title has an option to include context similar to [Archive Title](/vertex/widgets/dynamic-posts-templates/archive-title/). When *Include Context* is enabled (the default is "Yes"), the widget may prepend the post type or context to the title. For example, on a regular page it might show "Page: About Us", or on a post it could show "Post: How to Bake Bread". If you prefer to show just the title without any prefix, you can disable this:

<Aside type="tip">To remove the prefix, click on the dynamic tag icon next to the title field in the widget's settings and turn **Include Context** to "No". In most cases for pages, you will likely turn this off, since seeing "Page: About Us" is not usually desired on the frontend.</Aside>
